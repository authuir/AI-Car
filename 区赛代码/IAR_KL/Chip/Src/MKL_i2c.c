...
#define GY50_Thur_Set 1000

unsigned char MasterTransmission;
unsigned char SlaveID;

unsigned char device_id;   //WHO_AM_I

unsigned char angel_TX_DATA[4];  
unsigned char angel_BUF[8];                         //接收数据缓存区
char  angel_test=0; 
short angel_T_X,angel_T_Y,angel_T_Z;
...
{
	i2c_write_reg(I2C0 , L3G4200_Addr, CTRL_REG1, 0x0f);
	i2c_write_reg(I2C0 , L3G4200_Addr, CTRL_REG2, 0x00);
	i2c_write_reg(I2C0 , L3G4200_Addr, CTRL_REG3, 0x08);
	i2c_write_reg(I2C0 , L3G4200_Addr, CTRL_REG4, 0x30);
	i2c_write_reg(I2C0 , L3G4200_Addr, CTRL_REG5, 0x00);
...
void I2c_port_init()
{
     port_init(PTA6, ALT1 | IRQ_RISING | PULLDOWN ); 
     port_init(PTA6, ALT1 | IRQ_RISING | PULLDOWN ); 
}
...
{
   angel_BUF[0]=i2c_read_reg(I2C0 ,L3G4200_Addr,OUT_X_L);
   angel_BUF[1]=i2c_read_reg(I2C0 ,L3G4200_Addr,OUT_X_H);
   angel_T_X=	(angel_BUF[1]<<8)|angel_BUF[0];
 


   angel_BUF[2]=i2c_read_reg(I2C0 ,L3G4200_Addr,OUT_Y_L);
   angel_BUF[3]=i2c_read_reg(I2C0 ,L3G4200_Addr,OUT_Y_H);
   angel_T_Y=	(angel_BUF[3]<<8)|angel_BUF[2];
  

   angel_BUF[4]=i2c_read_reg(I2C0 ,L3G4200_Addr,OUT_Z_L);
   angel_BUF[5]=i2c_read_reg(I2C0 ,L3G4200_Addr,OUT_Z_H);
   angel_T_Z=	(angel_BUF[5]<<8)|angel_BUF[4];
}
