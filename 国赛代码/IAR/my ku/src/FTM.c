...
void QD_init_FTM()
 {
#if  1
     SIM_SCGC6 |= SIM_SCGC6_FTM1_MASK;//使能FTM0
     SIM_SCGC3 |= SIM_SCGC3_FTM2_MASK;//使能FTM0
     
     SIM_SCGC5 |= SIM_SCGC5_PORTA_MASK;//使能PORTA时钟
     PORT_Configuer(PORTA,8,6);
     PORT_Configuer(PORTA,9,6);
     
     
     FTM1_MOD = 0xFFFF;//设置上限，其实没啥用
     ...
     FTM1_CNT = 0;//初始清零
      ...
      PORT_Configuer(PORTA,10,6);
     PORT_Configuer(PORTA,11,6);
     
     
     FTM2_MOD = 0xFFFF;//设置上限，其实没啥用
...
{
	FTM_CnV_REG(FTMn[Ftmn], CHx) = (uint16)((float)(FTM_MOD_REG(FTMn[Ftmn]))*Duty-0.5);
...
void FTM_IC_Init(uint16 Ftmn_CHx_PTnx, FTM_Input_cfg cfg)
{
    ...
    case FTM_Rising_or_Falling: //上升沿、下降沿都触发
        FTM_CnSC_REG(FTMn[Ftmn], CHx) |=  ( FTM_CnSC_ELSB_MASK | FTM_CnSC_ELSA_MASK   ); //置1
        FTM_CnSC_REG(FTMn[Ftmn], CHx) &= ~( FTM_CnSC_MSB_MASK  | FTM_CnSC_MSA_MASK); //清0
        break;
    }
    FTM_MOD_REG(FTMn[Ftmn]) = 0XFFFF; 
