#include "all.h"
const unsigned char Chinese16X16[] = 
{
/*--  文字:  起  --*/
...
0x04,0x04,0x02,0xFE,0xAB,0xAB,0xAA,0xAA,0xAA,0xAB,0xAB,0xFE,0x02,0x04,0x04,0x00,
...
void LCD_Init(void)
 {
     GPIO_INIT(LCD_DC_PORT,LCD_DC_PORT_PIN,1);
     GPIO_INIT(LCD_D0_PORT,LCD_D0_PORT_PIN,1);
     GPIO_INIT(LCD_D1_PORT,LCD_D1_PORT_PIN,1);
     GPIO_INIT(LCD_RESET_PORT,LCD_RESET_PORT_PIN,1);
     reset_clear;
     SYSDelay_ms(50);
     reset_set;
...
//功能描述：写入一组标准ASCII字符串
//参数：显示的位置（x,y），y为页范围0～7，x为0~16要显示的字符串
//返回：无
//==============================================================  
...
void LCD_8x16Str_Reverse(uint8 x,uint8 y,uint8 ch[])
 {
     uint8 c=0,i=0,j=0;
     x*=8;
     y=2*y;
     ...
     for(k=8;k>0;k--)
      {
          ...
          for(j=128;j>0;j-=1)
           {
               datata=0;
               for(i=8*k-1;i>=8*(k-1);i-=1)
                {
                    if(date[i*X_MAX/64][j*Y_MAX/128] >threshold_left_next) 
                        datata=(datata|0x80)>>1;
                    else datata=(datata>>1);
                }
               LCD_WrDat(datata);	
           }
      }
