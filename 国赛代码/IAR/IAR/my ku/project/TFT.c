...
void show_image(uint16 x,uint16 y,uint16 hang,uint16 lie,uint8  date1[X_MAX+6][Y_MAX+15])
 {
     ...
     {	
		...
		for(j=0;j<lie;j++)
		{
            if(date1[X_MAX-i-1][Y_MAX-j-1]>160)
                LCD_WR_DATA(0xffff);
            else LCD_WR_DATA(0);
		}
	}    
...
void show_real_pic(uint16 x,uint16 y,uint16 hang,uint16 lie,uint8  date1[X_MAX+6][Y_MAX+15])
 {
     uint16 i,j;
     uint8 Redc,Greenc,Bluec;
     for(i=0;i<hang;i++)
     {	
		LCD_SetCursor(x,y+i);
		LCD_WriteRAM_Prepare();     			//开始写入GRAM	
		for(j=0;j<lie;j++)
		{
		    Redc = date1[X_MAX-i-1][Y_MAX-j-1]*0.121;
			Bluec = date1[X_MAX-i-1][Y_MAX-j-1]*0.121;
			Greenc = date1[X_MAX-i-1][Y_MAX-j-1]*0.246;
			LCD_WR_DATA((Redc<<11) | (Greenc<<5) | Bluec);
		}
	}   
 }
...
void show_2image()
 {
     ...
     {	
		...
		for(j=0;j<300;j++)
		{
            if(Get_Transform_Value(i,j)>threshold)
                LCD_WR_DATA(0xffff);
            else LCD_WR_DATA(0);
		}
	}    
...
void show_dot(uint16 T[500][2],uint16 line_Num,uint16 line_color)
 {
     uint16 i,j;
     for(j=0;j<3;j++)
      {
          for(i=0;i<line_Num;i++)
           {
               LCD_SetCursor(Y_MAX-1-T[i][1],X_MAX-1-T[i][0]+j-1);      				//设置光标位置 
               LCD_WriteRAM_Prepare(); 
               LCD_WR_DATA(line_color);
           }
      }
 }
extern uint16  RST1[X_MAX * Y_MAX];
extern uint16 RST0[X_MAX * Y_MAX]; 
void show_dot_change(uint16 T[500][2], uint16 line_Num, uint16 line_color)
{
	uint16 i, j;
	for (j = 0; j<3; j++)
	{
		for (i = 0; i<line_Num; i++)
		{
			LCD_SetCursor(Y_MAX_OUT - 1 - RST1[T[i][0] * Y_MAX + T[i][1]], X_MAX_OUT - 1 - RST0[T[i][0] * Y_MAX + T[i][1]] + j - 1);      				//设置光标位置 
			LCD_WriteRAM_Prepare();
			LCD_WR_DATA(line_color);
		}
	}
...
void show_dot_change2(uint16 T[500][2], uint16 line_Num, uint16 line_color)
{
	uint16 i, j;
	for (j = 0; j<3; j++)
	{
		for (i = 0; i<line_Num; i++)
		{
			LCD_SetCursor(Y_MAX_OUT - 1 - T[i][1], X_MAX_OUT - 1 - T[i][0] + j - 1);      				//设置光标位置 
			LCD_WriteRAM_Prepare();
			LCD_WR_DATA(line_color);
		}
	}
...
void show_line(float line_k,float line_b)
 {
	...
	for (j = 0; j<3; j++)
	{
		for (i = 0; i<X_MAX_OUT; i++)
		{
			LCD_SetCursor(Y_MAX_OUT-1-(uint16)(i*line_k+line_b),X_MAX_OUT-1+j-i);      				//设置光标位置 
			LCD_WriteRAM_Prepare();
			LCD_WR_DATA(GREEN);
		}
	}
