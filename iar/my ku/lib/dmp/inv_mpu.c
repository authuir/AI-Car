...
#define delay_ms    SYSDelay_us
#define get_ms      get_ms
//返回0表示正常,返回-1表示不正常
int8 i2cWrite( uint8 addr , uint8 REG_Address , uint8 len , uint8 *REG_data )
{
		uint32 I2C_NUM=1;
                if(I2C_WN( I2C_NUM, addr, REG_Address, len, REG_data))
                    return 0;
                else
                    //while(1);
                return -1;
}
int8 i2cRead( uint8 addr , uint8 REG_Address , uint8 len , uint8 *REG_data )
{
		uint32 I2C_NUM=1;
		if(I2C_RN( I2C_NUM, addr, REG_Address, len, REG_data))
                    return 0;
                else
                    //while(1);
                return -1;
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
        return -1;
    if (i2c_write(st.hw->addr, st.reg->i2c_mst, 1, data_iic)) //381K iic
        return -1;
    if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, data))
        return -1;
